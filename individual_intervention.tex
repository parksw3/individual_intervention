...
\subsection{Quantifying changes in the instantaneous generation-interval distribution}
...
First, under strength- and speed-like interventions, $\Rc(t)$ is given by:
\begin{align}
\Rc(t) &= \int F_t(\sigma) \dsigma, \\
&= \Ro \int_0^\infty S(t+\sigma) \PP(t + \sigma) \II(t+\sigma, \sigma) g_0(\sigma) \dsigma.
\label{eq:rct}
\end{align}
We can further rearrange this equation to obtain:
\begin{align}
\Rc(t) &= \int_0^\infty \Ri(t+\sigma) g(t+\sigma, \sigma) \dsigma.
\end{align}
