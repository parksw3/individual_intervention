...
Here, we focus on two idealized interventions, which we refer to as pure strength- and speed-based interventions, and later generalize them to describe strength- and speed-based interventions \citep{dushoff2021speed}.
...
First, we consider a pure strength-based intervention $\II(t, \tau) = \PP(t)$, which includes susceptible depletion $\PP(t) = S(t)$ as a special case.
...
In practice, the hazard of isolation depends not only on calendar time $t$ but also on the age of infection $\tau$---for example, we expect $h(t, \tau)$ to increase with $\tau$ for symptom-based interventions to reflect the increasing cumulative probability of developing symptoms.
...
Finally, we can decompose the generalized intervention function $\II(t, \tau)$ as a product of strength- and spied-based interventions: $\II(t, \tau) = \PP(t) \HH(t, \tau)$. 
Then, the dynamics of incidence $i(t)$ can be now written as:
\begin{align}
...
\label{eq:b2}
\end{equation}
In this case, we see that \eref{b2} also depends on the case reproduction number $\Rc$ because a cohort of infectors (infected at time $t-\tau$) who themselves generated more infections at time $t$ has a greater contribution towards the backward distribution.
...
\end{equation}
While this estimator is similar in form to previously proposed estimators \citep{fraser2007estimating}, the main difference is that it allows for the underlying instantaneous generation-interval distribution to vary to account for speed-based interventions.
...
such method accurately captures changes in $\Ri(t)$ under strength-based intervention, but not under speed-based intervention.
