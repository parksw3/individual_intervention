...
\begin{flushleft}{
	...
	\newline \em Confidential draft 3 Jan 2023
	}
}
\newline
\\
...
\jd{Took out “real-time” because it's a bit confusing: we don't see infections instantaneously. But I do understand why it was there.}
\jd{I wonder if things would read better if we didn't keep repeating “(t)”}
...
Finally, we link a simple SIR model with the renewal-equation framework and find that changes in the transmission rate are equivalent to a constant-strength intervention, whereas changes in the recovery rate are equivalent to a constant-speed intervention---we thus conclude that $\RR(t)$ is generally a better measure for characterizing disease spread under constant-strength interventions, whereas $r(t)$ is generally better under constant-speed interventions.
\jd{How does this generalize beyond SIR?}
...
\jd{Should we add units (1/days) to the left y-axes?}
\jd{Is it obvious why we pay more attention to forward kernels than instantaneous kernels? What would the answers be for the instantaneous kernels? They're not changed at all by strength-like interventions, and look like panel E for speed-like? Do we say this somewhere clearly enough?}
...
The constant-speed interventions always shorten realized generation intervals because they prevent late transmission (\fref{indpop}D--F).
...
\jd{I guess this is the answer to my instantaneous question above. We can make this \P\ explicitly focused on instantaneous intervals (put it in the topic s.), and describe a bit more how the change works (cf.~to F1).}
\jd{Made some changes to try to fix line numbering; behaviour seems inconsistent.}
...
\end{equation}
Even if the instantaneous generation-interval distribution $g_t(\tau)$ remains invariant across time, changes in transmission conditions (and therefore $\Ri(t)$) can change the shape of the forward generation-interval distribution;
...
\jd{Broken cite here apparently.}
...
\jd{Maybe just talk directly about incubation periods; I feel like I've been dancing around them. Say that we are for now neglecting these delays maybe?}
...
In particular, the popular R package \texttt{EpiEstim}, \cite{cori2013new} uses this approach while assuming that the underlying instantaneous distribution does not change over time; 
...
\jd{“Including” is awkward here. I think it underlines that we need to address the serial/generation thing above.}
...
(C) Instantaneous incidence under equivalent strength- and speed-based intervention scenarios (both scenarios give identical incidence trajectories).
...
\jd{Why “despite”? Maybe we should discuss what to emphasize here.}
...
In particular, $r(t)$ can always be reliably estimated (given high-quality incidence data) but does not accurately reflect changes in current conditions, except when changes are predominantly speed-based.
Conversely, $\RR(t)$ -- and particularly the instantaneous reproductive number $\Ri(t)$ -- provides an accurate reflection of current conditions in \emph{theory}, but can only be reliably estimated from incidence data alone when changes are primarily strength-based.
...
\jd{I feel the mixture of fractions and inversion exponents is very weird. My first choice would be $0.3/\textrm{day}$ and my second would be $0.3\textrm{day}^{-1}$. I don't feel strongly about day vs.~days.}
\begin{equation}
...
Since we assume that incidence is known exactly until time $t^\ast$ we can estimate the transmission rate $\beta^\ast(t)$ (with fixed $\gamma(t)=\gamma(0)$) that gives an identical incidence trajectory until time $t^\ast$.
\jd{Do we need  $t^\ast$?}
