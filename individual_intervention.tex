...
\jd{Is it worth trying to be clear that classic \Ri\ doesn't actually depend on when the person gets infected, only on when the conditions are held constant at?}
...
\jd{susceptible depletion is often seen as dynamically intrinsic. just saying.}
...
Now, consider the forward kernel $F_t(\tau)$, which represents the rate at which an individual infected at time $t$ generates secondary cases $\tau$ time units after infection: 
\jd{This “rate” seems very good; make sure we use it above as well.}
\begin{equation}
...
\jd{I would like to expand this treatment a bit. What would you think of introducing the current \eref{fkernel} in two steps?}

\jd{I am not seeing how this \P\ works. It shows $g$ makes sense in the linearized case -- but everything makes sense in the linearized case; it's only indirect evidence for indirectness and I think makes things less clear? Hard for me to tell.}
...
\jd{Should we outline pure strength and pure speed first? Most individual-level interventions are more mixed according to the original idea. Sorry that I'm waffling a bit here.}
...
Our study has important practical implications for describing disease dynamics and communicating model estimates.
